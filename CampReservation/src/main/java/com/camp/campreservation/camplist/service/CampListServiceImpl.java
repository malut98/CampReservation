...
public class CampListServiceImpl implements CampListService {
	...
	@Override
	public List<CampDBDto> getBestList(Model model, String pagenum, String contentnum) {
		Pagination pagemaker = new Pagination();
		int cpagenum = Integer.parseInt(pagenum);
		int ccontentnum = Integer.parseInt(contentnum);
		pagemaker.setTotalcount(campListMapper.selectAllCount()); // mapper 전체 게시글 개수를 지정한다
        ...
        model.addAttribute("page",pagemaker);
		return campListMapper.getBestList(pagemaker.getPagenum()*6,pagemaker.getContentnum());
	}
	...
	@Override
	public List<CampDBDto> getPetList(Model model,String pagenum, String contentnum) {
		Pagination pagemaker = new Pagination();
		int cpagenum = Integer.parseInt(pagenum);
		int ccontentnum = Integer.parseInt(contentnum);
		pagemaker.setTotalcount(campListMapper.selectPetCount()); // mapper 전체 게시글 개수를 지정한다
        pagemaker.setPagenum(cpagenum-1);   // 현재 페이지를 페이지 객체에 지정한다 -1 을 해야 쿼리에서 사용할수 있다
        pagemaker.setContentnum(ccontentnum); // 한 페이지에 몇개씩 게시글을 보여줄지 지정한다.
        pagemaker.setCurrentblock(cpagenum); // 현재 페이지 블록이 몇번인지 현재 페이지 번호를 통해서 지정한다.
        pagemaker.setLastblock(pagemaker.getTotalcount()); // 마지막 블록 번호를 전체 게시글 수를 통해서 정한다.
        ...
        model.addAttribute("page",pagemaker);
		return campListMapper.getPetList(pagemaker.getPagenum()*6,pagemaker.getContentnum());
	}
	
	@Override
	public int selectPetCount() {
		return campListMapper.selectPetCount();
	}
	...
	@Override
	public List<CampDBDto> campRecommen(int camp_id, String camp_se) {
		return campListMapper.campRecommen(camp_id, camp_se);
	}
	...
	@Override
	public List<CampDBDto> searchCamp(Model model, String key, String pagenum, String contentnum, String name) {
		Pagination pagemaker = new Pagination();
		int cpagenum = Integer.parseInt(pagenum);
		int ccontentnum = Integer.parseInt(contentnum);
		pagemaker.setTotalcount(campListMapper.searchCampCount(key, name)); // mapper 전체 게시글 개수를 지정한다
        pagemaker.setPagenum(cpagenum-1);   // 현재 페이지를 페이지 객체에 지정한다 -1 을 해야 쿼리에서 사용할수 있다
        pagemaker.setContentnum(ccontentnum); // 한 페이지에 몇개씩 게시글을 보여줄지 지정한다.
        pagemaker.setCurrentblock(cpagenum); // 현재 페이지 블록이 몇번인지 현재 페이지 번호를 통해서 지정한다.
        pagemaker.setLastblock(pagemaker.getTotalcount()); // 마지막 블록 번호를 전체 게시글 수를 통해서 정한다.
	...
	@Override
	public int searchCampCount(String key, String name) {
		return campListMapper.searchCampCount(key, name);
	}
}
