...
public class CommunityService {
	...
	public List<CommunityDto> searchboardlist(Model model, String pagenum, String contentnum, CommunityDto dto, String searchOption, String keyword){
		Pagination pagemaker = new Pagination();
		System.out.println("검색옵션:" + searchOption);
		System.out.println("키워드:" + keyword);
		int cpagenum = Integer.parseInt(pagenum);
		int ccontentnum = Integer.parseInt(contentnum);
		List<CommunityDto> cList = null;
		pagemaker.setTotalcount(cm.selectsearchmenucount(dto, searchOption, keyword)); // mapper 전체 게시글 개수를 지정한다
        pagemaker.setPagenum(cpagenum-1);   // 현재 페이지를 페이지 객체에 지정한다 -1 을 해야 쿼리에서 사용할수 있다
        pagemaker.setContentnum(ccontentnum); // 한 페이지에 몇개씩 게시글을 보여줄지 지정한다.
        pagemaker.setCurrentblock(cpagenum); // 현재 페이지 블록이 몇번인지 현재 페이지 번호를 통해서 지정한다.
        pagemaker.setLastblock(pagemaker.getTotalcount()); // 마지막 블록 번호를 전체 게시글 수를 통해서 정한다.
        ...
        if(ccontentnum == 5){//선택 게시글 수
        	cList = cm.searchlist(dto,searchOption,keyword,pagemaker.getPagenum()*5,pagemaker.getContentnum());
        }else if(ccontentnum == 10){
        	cList = cm.searchlist(dto,searchOption,keyword,pagemaker.getPagenum()*10,pagemaker.getContentnum());
        }else if(ccontentnum ==20){
        	cList = cm.searchlist(dto,searchOption,keyword,pagemaker.getPagenum()*20,pagemaker.getContentnum());
        }
        System.out.println(dto.getCom_cate());
       System.out.println(pagemaker.getTotalcount());
        model.addAttribute("clist",cList);
